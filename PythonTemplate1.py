#https://atcoder.jp/contests/abc084/submissions/57359657
...
def perr(*l):
    if _log:
        ...
        print(*l, '\033[0m', file=sys.stderr, flush = True)
...
class Ru:
    ...
    def sum(self, l, r):
        if self.b == None:
            self.b = [0] * (self.n + 1)
            for i in range(n):
                self.b[i+1] = (self.b[i] + self.a[i])
        return self.b[r] - self.b[l]
...
for st in range(n):
    ...
    for j in range(n-1):
        if n-1 <= st + j:
            break
        if now <= a[st + j][1]:
            now = a[st + j][1] + a[st + j][0]
        else:
            now = kiriage(now, a[st + j][2]) * a[st + j][2] + a[st + j][0]
    ans.append(now)
for e in ans:
    print(e)
